...
def _seed_for(scenario: str) -> int:
    return int(hashlib.sha256(scenario.encode()).hexdigest(), 16) % (2 ** 32)


def generate_series(scenario: str, start_time: datetime | None = None) -> pd.DataFrame:
    if scenario not in SCENARIO_PARAMS:
        raise ValueError(f"Unknown scenario: {scenario}")
    params = SCENARIO_PARAMS[scenario]
    rng = np.random.default_rng(_seed_for(scenario))
    ...
    solar_kw = SOLAR_CAPACITY_KW * (daylight ** 1.3) * (1 - 0.85 * cloud_cover)
    solar_kw = np.clip(solar_kw + rng.normal(0, 0.4, TOTAL_POINTS), 0, SOLAR_CAPACITY_KW)

    # Wind power ~ cube of wind speed, capped at rated capacity.
    wind_power_raw = 0.015 * wind_speed ** 3
    wind_kw = np.clip(wind_power_raw, 0, WIND_CAPACITY_KW)
    wind_kw = np.clip(wind_kw + rng.normal(0, 0.3, TOTAL_POINTS), 0, WIND_CAPACITY_KW)
    ...
    base_load = 6
    demand_kw = (base_load + morning_peak + evening_peak) * params["demand_scale"]
